...
m = Basemap(projection='merc', llcrnrlon=lon0, llcrnrlat=lat0,
            urcrnrlon=lon1, urcrnrlat=lat1, resolution='l', ax=ax_list[0])
...
for group in range(n_groups_lats):
    ...
    zone_bbox_style = dict(boxstyle="round", ec=(0, 0, 0, 0.9),
                           fc=(1., 1, 1, 0.6))
...
em = HawkesEM(kernel_discretization=kernel_discretization,
              tol=1e-9, max_iter=1000, print_every=200)
em.fit(events)
